...
MissionCondition1 = [ "WIN1",{ 
		...
		&& !alive TANK
		&& {call fnc_CheckPlayersReturned}
        }, "Base destroyed, tank destroyed" ];
		
MissionCondition2 = [ "WIN2",{ 
        [ [TRG1, CITY], "east", "primaryWeapon _x != ''", "< 3"] call dzn_fnc_ccUnits
		&& !alive TANK
		&& {call fnc_CheckPlayersReturned}
        }, "Base destroyed, tank destroyed, New Sobor clear" ];
		
MissionCondition3 = [ "WIPED1",{ 
		[ CITY, "east", "", "< 3"] call dzn_fnc_ccUnits
		&& {call fnc_CheckPlayersReturned}
        }, "Only New Sobor clear" ];		
		
MissionCondition4 = [ "WIPED2", { call fnc_isAllDead }, "All dead", 30 ];
